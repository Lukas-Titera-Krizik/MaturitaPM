...
<div class="Nadpis">
    <?php
    echo ' 
  <p>' . $_SESSION['mainCategoryOpt'] . ' >> ' . $_SESSION['categoryOpt'] . '</p>
  ';
    ?>
...
<div class="ProhlizecProduktu">
    <?php
          $category = $_SESSION['mainCategoryOpt'];
          $subcategory = $_SESSION['categoryOpt'];
          //STEJNÝ POSTUP JAKO V NAVBARU, KDY SE NEJDŘÍV VYBÍRAJÍ VŠECHNY PRODUKTY S PŘÍSLUŠNOU HLAVNÍ KATEGORIÍ
          $sql = "SELECT PRODUCT.idPRODUCT, PRODUCT.NAME, PRODUCT_has_CATEGORY.PRODUCT_idPRODUCT, PRODUCT_has_CATEGORY.CATEGORY_idCATEGORY, CATEGORY.NAME AS CATEGORYNAME
FROM PRODUCT_has_CATEGORY
JOIN PRODUCT ON PRODUCT.idPRODUCT = PRODUCT_has_CATEGORY.PRODUCT_idPRODUCT
JOIN CATEGORY ON CATEGORY.idCATEGORY = PRODUCT_has_CATEGORY.CATEGORY_idCATEGORY
WHERE CATEGORY.NAME = '$category'";
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            //POKUD JE VÝSLEDEK PŘEDEŠLÉ FILTRACE NENULOVÝ TAK SE PROJDE DRUHOU FILTRACÍ, TENTOKRÁT SE SUBKATEGORIÍ
            $sql = "SELECT PRODUCT.idPRODUCT, PRODUCT.NAME, PRODUCT.DESCRIPTION, PRODUCT.SIZE, PRODUCT.PRICE, PRODUCT_has_CATEGORY.PRODUCT_idPRODUCT, PICTURE.PRODUCT_idPRODUCT, PICTURE.DIRECTORY, PRODUCT_has_CATEGORY.CATEGORY_idCATEGORY, CATEGORY.NAME AS CATEGORYNAME
FROM PRODUCT_has_CATEGORY
JOIN PRODUCT ON PRODUCT.idPRODUCT = PRODUCT_has_CATEGORY.PRODUCT_idPRODUCT
JOIN CATEGORY ON CATEGORY.idCATEGORY = PRODUCT_has_CATEGORY.CATEGORY_idCATEGORY
JOIN PICTURE ON PICTURE.PRODUCT_idPRODUCT = PRODUCT_has_CATEGORY.PRODUCT_idPRODUCT
WHERE CATEGORY.NAME ='" . $subcategory . "'  AND PRODUCT.NAME = '" . $row["NAME"] . "'";
            $result2 = $conn->query($sql);
            if ($result2->num_rows > 0) {
                while($row2 = $result2->fetch_assoc()) {


                    //ZDE SE OVĚŘUJE, ŽE PRODUKT UŽ NĚKDO NEZAKOUPIL, V PŘÍPADĚ, ŽE ANO, TAK SE NEUKÁŽE
                    $sqlZakoupeni = "SELECT * FROM ORDER_has_PRODUCT WHERE PRODUCT_idPRODUCT = '" . $row2['idPRODUCT'] . "'";
                    ...
                    if($overeniZakoupeni->num_rows < 1){
                        ...
                        <img class="obrazek" src="' . $row2["DIRECTORY"] . '" alt="' . $row2["NAME"] . '" >
                        </div>
                        <div class="ProduktPopisek">
                        <a class="ProduktNazev">' . $row2["NAME"] . '</a>
                        <a>' . $row2["DESCRIPTION"] . '</a>
                        <a>Velikost: ' . $row2["SIZE"] . '</a>
                        </div>';


                        echo '<div class="ProduktTlacitko">
                          <p class="ProduktCena">'. $row2["PRICE"] .' Kč</p>';

                        $vKosiku = false;
                        //TATO FUNKCE SLOUŽÍ PRO KONTROLU JESTLI PRODUKT JIŽ NENÍ V KOŠÍKU, V PŘÍPADĚ ŽE ANO, TAK SE NEUMOŽNÍ UŽIVATELI PŘIDAT HO ZNOVU
                        if(isset($_SESSION['cart']) && !empty($_SESSION['cart'])){
                            foreach($_SESSION['cart'] as $itemID){
                                if($itemID == $row2['idPRODUCT'] && $vKosiku == false){
                                    $vKosiku = true;
                                }
                            }
                        }
                        ...
                        <input type="hidden" name="idPRODUCT" value="'. $row2["idPRODUCT"] .'">
