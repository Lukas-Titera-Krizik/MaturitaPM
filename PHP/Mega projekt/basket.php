...
<head>
  <meta charset=utf-8>
  <title>Košík | Titolo Eclat</title>
    <?php
    session_start();
    include 'conn.php';
...
<div class="Obsah">
    ...
    if(isset($_SESSION['cart']) && !empty($_SESSION['cart'])){
        ...
        foreach($_SESSION['cart'] as $itemID){
            //PRO KAŽDÝ OBJEKT V KOŠÍKU SE VYBEROU VŠECHNY ÚDAJE
            $sql = "SELECT PRODUCT.NAME, PRODUCT.PRICE, PRODUCT.SIZE, PRODUCT.DESCRIPTION, PICTURE.DIRECTORY FROM PRODUCT
                                                                      JOIN PICTURE ON PICTURE.PRODUCT_idPRODUCT = PRODUCT.idPRODUCT
                                                                      WHERE PRODUCT.idPRODUCT = '" . $itemID . "'";
            $result = $conn->query($sql);
            ...
            echo '<div class="ProduktTlacitko">
                        ...
                        <form class="tlacitko" action="remove_from_cart_script.php">
                        <input type="hidden" name="idPRODUCT" value="'. $row["idPRODUCT"] .'">
                        <input type="submit" name="submit" class="PridatButton" value="Odstranit">
                        </form>
                        </div>
                        </div>';

            $cenaCelkem = $cenaCelkem + $row["PRICE"];
  ...
  <div class="Produkt">
    ...
    <form method="post" action="remove_from_cart_script.php">
    <input type="hidden" name="index" class="CenaCelkemBt" value="'. $index .'">
    <input type="submit" name="submit" class="CenaCelkemBt" value="Odstranit">
</form>
  </div>
            ';
            $index = $index + 1;*/
        }
        //TLAČÍTKO ODKAZUJÍCÍ A ODEBRÁNÍ POLOŽKY Z KOŠÍKU
        echo '
        <form method="post" action="remove_from_cart_script.php">
        <input type="submit" name="celyKosik" class="CenaCelkemBt" value="Odstranit vše">
    </form>
        ';
    }
    ?>
...
<div class="Obsah">
  <div class="CenaCelkem">
    <p class="cenaCelkem">Celková cena: <?php echo $cenaCelkem ?> Kč</p>
  </div>
    <form method="post" action="continue_order_script.php">
        <input type="submit" name="submit" class="CenaCelkemBt" value="Objednat">
    </form>
