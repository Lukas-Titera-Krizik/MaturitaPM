    ...
    <nav id="leftNav">
        <div class="dropdown">
            ...
            <form class="dropdown-content" action="categorySelect.php" method="post">
                <?php
                //VYBERE VŠECHNY NÁZVY KATEGORIÍ
                $sql = "SELECT NAME FROM CATEGORY";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        //POKUD JE NÁZEV JEDEN ZE ZMÍNĚNÝCH, TAK SE NEVYPÍŠE
                        if($row['NAME'] != "Pánské" && $row['NAME'] != "Dámské" && $row['NAME'] != "Dětské"){


                            $category = "Pánské";
                            $subcategory = $row['NAME'];
                            //VÝBĚR VŠECH DAT Z KATEGORIE V TOMTO PŘÍPANĚ "PÁNSKÉ"
...
WHERE CATEGORY.NAME = '$category'";
                            ...
                            if ($result1->num_rows > 0) {
                                while($row1 = $result1->fetch_assoc()) {
                                    //Z VÝSLEDNÝCH DAT PROVEDEME DRUHOU FILTRACI, TENTOKRÁT S SUBKATEGORIÍ A NÁZVU PRODUKTU Z PŘEDEŠLÉ FILTRACE
...
WHERE CATEGORY.NAME ='" . $subcategory . "'  AND PRODUCT.NAME = '" . $row1["NAME"] . "'";
                                    ...
                                    //POKUD VÝSLEDEK Z VYHLEDÁNÍ JE VETŠÍ NEŽ NULA, TAK SE UŽIVATELI OBJEVÍ MOŽNOST NA DANNOU SUBKATEGORII KLIKNOUT
...
WHERE CATEGORY.NAME ='" . $subcategory . "'  AND PRODUCT.NAME = '" . $row1["NAME"] . "'";
    ...
    <script>
        ...
        //TENTO SCRIPT HLÍDÁ JESTLI JE ZMÁČNUT ENTER, V TOM PŘÍPADĚ VYHLEDÁ CO JE ZADÁNO VE VYHLEDÁVACÍM OKNĚ
